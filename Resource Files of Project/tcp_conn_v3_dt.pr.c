/* Process model C form file: tcp_conn_v3_dt.pr.c */
/* Portions of this file copyright 1992-2004 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char tcp_conn_v3_dt_pr_c [] = "MIL_3_Tfile_Hdr_ 110A 30A op_runsim 7 46E9AD7F 46E9AD7F 1 zeus momueti 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 b56 1                                                                                                                                                                                                                                                                                                                                                                                                            ";
#include <string.h>
...
typedef struct
	...
	TcpT_Ip_Encap_Req_Ici_Info			ip_encap_ici_info                               ;	/* Structure for storing the ip_encap_ind_v6 ici used to send */
	                        		                                                	/* packets to ip_encap.											                            */
	} tcp_conn_v3_dt_state;

#define pr_state_ptr            		((tcp_conn_v3_dt_state*) (OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#  define FIN_PREAMBLE_DEC	tcp_conn_v3_dt_state *op_sv_ptr;
#if defined (OPD_PARALLEL)
#  define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((tcp_conn_v3_dt_state *)(sim_context_ptr->_op_mod_state_ptr));
#else
...
/* Addition by Modupe Omueti */
double					rho;

/* Addition by Modupe Omueti ends */
...
tcp_rtt_measurements_update (double measurement_start_time)
	...
	if (rtt_active && tcp_seq_lt (rtt_seq, seg_ack))
		...
		retrans_rto = retrans_rtt + MAX(rtt_dev_coef * retrans_rtt_dev,timer_gran);
...
tcp_cwnd_update (TcpT_Size acked_bytes)
	...
	if (tcp_flavor == TcpC_Flavor_New_Reno)
		...
		if (tcp_seq_lt (snd_una, snd_recover))
			...
			if (cwnd <= acked_bytes)
				...
				cwnd = cwnd - acked_bytes + (2 * snd_mss);
				}			
	...
	if (ev_ptr->flags & TCPC_FLAG_ECE)
		...
		if (cwnd < ssthresh)
			...
			/* Addition by Modupe Omueti */
			if ((cwnd <= ssthresh /4)  &&(snd_recover==0) &&(snd_max - snd_una < snd_wnd /4))
				{
				if (rho >= 15)
					{
					cwnd += ((ceil)((sqrt(rho))/4))* snd_mss;
					}
				else
					{
					cwnd += snd_mss;
			...
			else if ((cwnd <= ssthresh /4)  &&(snd_recover!=0) &&(snd_max - snd_una < snd_wnd /4))
				{
				cwnd += snd_mss;
				}
			else if ((cwnd > ssthresh /4) && (cwnd <= ssthresh / 2) &&(snd_recover==0) &&(snd_max - snd_una < snd_wnd /4))
				{
				if (rho >= 15)
					{
					cwnd += ((ceil)((sqrt(rho))/4)) * snd_mss;
			...
			else if ((cwnd > ssthresh /4) && (snd_recover==0) &&(snd_max - snd_una >= snd_wnd /4)&& (snd_max - snd_una < snd_wnd/2))
				{
				if (rho >= 15)
					{
					cwnd += ((ceil)((sqrt(rho))/4)) * snd_mss;
			...
			else if ((cwnd > ssthresh /2) && (snd_recover==0) &&(snd_max - snd_una >= snd_wnd /4)&& (snd_max - snd_una < snd_wnd/2))
				{
				if (rho >= 15)
					{
					cwnd += ((ceil)((sqrt(rho))/4)) * snd_mss;
			...
			if (!((SACK_PERMITTED) && (tcp_seq_gt (scoreboard_ptr->recovery_end, seg_ack))))
				...
				if (rho >= 15)
					{
					if (((tcp_seq_lt (snd_una, snd_recover)) && (snd_recover != 0)) ||(snd_max - snd_una >= snd_wnd/2))
						{
						cwnd += snd_mss * snd_mss / cwnd;
						}
					else if (((tcp_seq_gt (snd_una, snd_recover)) && (snd_recover != 0))||(snd_max - snd_una < snd_wnd/2))
						{
						cwnd += (sqrt(rho))/2 * snd_mss * snd_mss / cwnd;
						}
					}
				else
					cwnd += snd_mss * snd_mss / cwnd;
...
tcp_timeout_retrans (void* PRG_ARG_UNUSED (input_ptr), int PRG_ARG_UNUSED (code))
	...
	if (tcp_seq_lt (snd_nxt, snd_recover))
		snd_recover = snd_nxt;
	...
	/* Compute the next RTO expiration time.	*/
	next_timeout_time = Tcp_Slowtimo_Next_Timeout_Time_Obtain (current_time + 0.2, current_rto, timer_gran);
...
tcp_snd_data_process (void)
	...
	unacked_data_exist_before_send_started = tcp_seq_lt (snd_una, snd_max) ? OPC_TRUE : OPC_FALSE;
...
tcp_una_buf_process (Boolean unacked_data_exist)
	...
	/* Addition by Modupe Omueti */
	if (bytes_to_be_resent > (3 * snd_mss))
		bytes_to_be_resent = 3 * snd_mss;
	/* Addition by Modupe Omueti ends */
...
tcp_snd_total_data_size (Boolean unacked_data_exist)
	...
	if(rho >= 15)
		{
		if ((snd_max - snd_una) >= snd_wnd)
			{
			snd_wnd += 0;
			}
		else if ((cwnd > ssthresh) && ((snd_max - snd_una) < snd_wnd)) 
			{
			if ((snd_nxt >= snd_max) && (snd_recover < snd_una) && (snd_recover == 0))
				{
				snd_wnd = snd_wnd + ((int)(rtt_dev_gain * rho) * snd_mss);
				}
			else if((tcp_seq_le (snd_una, snd_recover)) && (snd_recover != 0))
				{
				snd_wnd += snd_mss;
				}
			else
...
extern "C" {
#endif
	void tcp_conn_v3_dt (OP_SIM_CONTEXT_ARG_OPT);
	VosT_Obtype _op_tcp_conn_v3_dt_init (int * init_block_ptr);
	VosT_Address _op_tcp_conn_v3_dt_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype, int);
	void _op_tcp_conn_v3_dt_diag (OP_SIM_CONTEXT_ARG_OPT);
	void _op_tcp_conn_v3_dt_terminate (OP_SIM_CONTEXT_ARG_OPT);
	void _op_tcp_conn_v3_dt_svar (void *, const char *, void **);
...
void
tcp_conn_v3_dt (OP_SIM_CONTEXT_ARG_OPT)
	{

#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (tcp_conn_v3_dt ());
		{
		/* Temporary Variables */
		...
		FSM_ENTER ("tcp_conn_v3_dt")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "tcp_conn_v3_dt [init enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [init enter execs]", state0_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (0, "init", "tcp_conn_v3_dt [init exit execs]")


			/** state (init) transition processing **/
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [init trans conditions]", state0_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 1, state1_enter_exec, ;, "OPEN_PASSIVE", "", "init", "LISTEN", "tcp_conn_v3_dt [init -> LISTEN : OPEN_PASSIVE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, SEND_SYN;, "OPEN_ACTIVE", "SEND_SYN", "init", "SYN_SENT", "tcp_conn_v3_dt [init -> SYN_SENT : OPEN_ACTIVE / SEND_SYN]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (1, "LISTEN", state1_enter_exec, "tcp_conn_v3_dt [LISTEN enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LISTEN enter execs]", state1_enter_exec)
				{
			...
			FSM_EXIT (3,"tcp_conn_v3_dt")


			/** state (LISTEN) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "LISTEN", "tcp_conn_v3_dt [LISTEN exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LISTEN exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LISTEN trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, SEND_SYN_ACK;, "RCV_SYN", "SEND_SYN_ACK", "LISTEN", "SYN_RCVD", "tcp_conn_v3_dt [LISTEN -> SYN_RCVD : RCV_SYN / SEND_SYN_ACK]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, SEND_SYN;, "SEND", "SEND_SYN", "LISTEN", "SYN_SENT", "tcp_conn_v3_dt [LISTEN -> SYN_SENT : SEND / SEND_SYN]")
				FSM_CASE_TRANSIT (2, 11, state11_enter_exec, ;, "CLOSE", "", "LISTEN", "CLOSED", "tcp_conn_v3_dt [LISTEN -> CLOSED : CLOSE / ]")
				FSM_CASE_TRANSIT (3, 3, state3_enter_exec, SEND_SYN;, "OPEN_ACTIVE", "SEND_SYN", "LISTEN", "SYN_SENT", "tcp_conn_v3_dt [LISTEN -> SYN_SENT : OPEN_ACTIVE / SEND_SYN]")
				FSM_CASE_TRANSIT (4, 1, state1_enter_exec, ;, "default", "", "LISTEN", "LISTEN", "tcp_conn_v3_dt [LISTEN -> LISTEN : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (2, "SYN_RCVD", state2_enter_exec, "tcp_conn_v3_dt [SYN_RCVD enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_RCVD enter execs]", state2_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (5,"tcp_conn_v3_dt")


			/** state (SYN_RCVD) exit executives **/
			FSM_STATE_EXIT_UNFORCED (2, "SYN_RCVD", "tcp_conn_v3_dt [SYN_RCVD exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_RCVD exit execs]", state2_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_RCVD trans conditions]", state2_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 4, state4_enter_exec, ;, "RCV_ACK_OF_SYN", "", "SYN_RCVD", "ESTAB", "tcp_conn_v3_dt [SYN_RCVD -> ESTAB : RCV_ACK_OF_SYN / ]")
				FSM_CASE_TRANSIT (1, 5, state5_enter_exec, SEND_FIN;, "CLOSE", "SEND_FIN", "SYN_RCVD", "FINWAIT1", "tcp_conn_v3_dt [SYN_RCVD -> FINWAIT1 : CLOSE / SEND_FIN]")
				FSM_CASE_TRANSIT (2, 1, state1_enter_exec, ;, "PASSIVE_RESET", "", "SYN_RCVD", "LISTEN", "tcp_conn_v3_dt [SYN_RCVD -> LISTEN : PASSIVE_RESET / ]")
				FSM_CASE_TRANSIT (3, 9, state9_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "SYN_RCVD", "CLOSE_WAIT", "tcp_conn_v3_dt [SYN_RCVD -> CLOSE_WAIT : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (4, 2, state2_enter_exec, ;, "default", "", "SYN_RCVD", "SYN_RCVD", "tcp_conn_v3_dt [SYN_RCVD -> SYN_RCVD : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (3, "SYN_SENT", state3_enter_exec, "tcp_conn_v3_dt [SYN_SENT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_SENT enter execs]", state3_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (7,"tcp_conn_v3_dt")


			/** state (SYN_SENT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (3, "SYN_SENT", "tcp_conn_v3_dt [SYN_SENT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_SENT exit execs]", state3_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [SYN_SENT trans conditions]", state3_trans_conds)
			FSM_INIT_COND (RCV_SYN)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, SEND_ACK;, "RCV_SYN", "SEND_ACK", "SYN_SENT", "SYN_RCVD", "tcp_conn_v3_dt [SYN_SENT -> SYN_RCVD : RCV_SYN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 4, state4_enter_exec, SEND_ACK;, "RCV_SYN_ACK", "SEND_ACK", "SYN_SENT", "ESTAB", "tcp_conn_v3_dt [SYN_SENT -> ESTAB : RCV_SYN_ACK / SEND_ACK]")
				FSM_CASE_TRANSIT (2, 11, state11_enter_exec, ;, "CLOSE", "", "SYN_SENT", "CLOSED", "tcp_conn_v3_dt [SYN_SENT -> CLOSED : CLOSE / ]")
				FSM_CASE_TRANSIT (3, 3, state3_enter_exec, ;, "default", "", "SYN_SENT", "SYN_SENT", "tcp_conn_v3_dt [SYN_SENT -> SYN_SENT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (4, "ESTAB", state4_enter_exec, "tcp_conn_v3_dt [ESTAB enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [ESTAB enter execs]", state4_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (9,"tcp_conn_v3_dt")


			/** state (ESTAB) exit executives **/
			FSM_STATE_EXIT_UNFORCED (4, "ESTAB", "tcp_conn_v3_dt [ESTAB exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [ESTAB exit execs]", state4_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [ESTAB trans conditions]", state4_trans_conds)
			FSM_INIT_COND (CLOSE)
			FSM_TEST_COND (RCV_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 5, state5_enter_exec, SEND_FIN;, "CLOSE", "SEND_FIN", "ESTAB", "FINWAIT1", "tcp_conn_v3_dt [ESTAB -> FINWAIT1 : CLOSE / SEND_FIN]")
				FSM_CASE_TRANSIT (1, 9, state9_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "ESTAB", "CLOSE_WAIT", "tcp_conn_v3_dt [ESTAB -> CLOSE_WAIT : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "default", "", "ESTAB", "ESTAB", "tcp_conn_v3_dt [ESTAB -> ESTAB : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (5, "FINWAIT1", state5_enter_exec, "tcp_conn_v3_dt [FINWAIT1 enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT1 enter execs]", state5_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (11,"tcp_conn_v3_dt")


			/** state (FINWAIT1) exit executives **/
			FSM_STATE_EXIT_UNFORCED (5, "FINWAIT1", "tcp_conn_v3_dt [FINWAIT1 exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT1 exit execs]", state5_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT1 trans conditions]", state5_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 8, state8_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "FINWAIT1", "CLOSING", "tcp_conn_v3_dt [FINWAIT1 -> CLOSING : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "RCV_ACK_OF_FIN", "", "FINWAIT1", "FINWAIT2", "tcp_conn_v3_dt [FINWAIT1 -> FINWAIT2 : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (2, 7, state7_enter_exec, ;, "RCV_FIN_ACK_OF_FIN", "", "FINWAIT1", "TIME_WAIT", "tcp_conn_v3_dt [FINWAIT1 -> TIME_WAIT : RCV_FIN_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (3, 5, state5_enter_exec, ;, "default", "", "FINWAIT1", "FINWAIT1", "tcp_conn_v3_dt [FINWAIT1 -> FINWAIT1 : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (6, "FINWAIT2", state6_enter_exec, "tcp_conn_v3_dt [FINWAIT2 enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT2 enter execs]", state6_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (13,"tcp_conn_v3_dt")


			/** state (FINWAIT2) exit executives **/
			FSM_STATE_EXIT_UNFORCED (6, "FINWAIT2", "tcp_conn_v3_dt [FINWAIT2 exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT2 exit execs]", state6_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [FINWAIT2 trans conditions]", state6_trans_conds)
			FSM_INIT_COND (RCV_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, SEND_ACK;, "RCV_FIN", "SEND_ACK", "FINWAIT2", "TIME_WAIT", "tcp_conn_v3_dt [FINWAIT2 -> TIME_WAIT : RCV_FIN / SEND_ACK]")
				FSM_CASE_TRANSIT (1, 6, state6_enter_exec, ;, "default", "", "FINWAIT2", "FINWAIT2", "tcp_conn_v3_dt [FINWAIT2 -> FINWAIT2 : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (7, "TIME_WAIT", state7_enter_exec, "tcp_conn_v3_dt [TIME_WAIT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [TIME_WAIT enter execs]", state7_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (15,"tcp_conn_v3_dt")


			/** state (TIME_WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (7, "TIME_WAIT", "tcp_conn_v3_dt [TIME_WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [TIME_WAIT exit execs]", state7_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [TIME_WAIT trans conditions]", state7_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 11, state11_enter_exec, ;, "TIMEOUT", "", "TIME_WAIT", "CLOSED", "tcp_conn_v3_dt [TIME_WAIT -> CLOSED : TIMEOUT / ]")
				FSM_CASE_TRANSIT (1, 7, state7_enter_exec, ;, "default", "", "TIME_WAIT", "TIME_WAIT", "tcp_conn_v3_dt [TIME_WAIT -> TIME_WAIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (8, "CLOSING", state8_enter_exec, "tcp_conn_v3_dt [CLOSING enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSING enter execs]", state8_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (17,"tcp_conn_v3_dt")


			/** state (CLOSING) exit executives **/
			FSM_STATE_EXIT_UNFORCED (8, "CLOSING", "tcp_conn_v3_dt [CLOSING exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSING exit execs]", state8_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSING trans conditions]", state8_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 7, state7_enter_exec, ;, "RCV_ACK_OF_FIN", "", "CLOSING", "TIME_WAIT", "tcp_conn_v3_dt [CLOSING -> TIME_WAIT : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (1, 8, state8_enter_exec, ;, "default", "", "CLOSING", "CLOSING", "tcp_conn_v3_dt [CLOSING -> CLOSING : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (9, "CLOSE_WAIT", state9_enter_exec, "tcp_conn_v3_dt [CLOSE_WAIT enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSE_WAIT enter execs]", state9_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (19,"tcp_conn_v3_dt")


			/** state (CLOSE_WAIT) exit executives **/
			FSM_STATE_EXIT_UNFORCED (9, "CLOSE_WAIT", "tcp_conn_v3_dt [CLOSE_WAIT exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSE_WAIT exit execs]", state9_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSE_WAIT trans conditions]", state9_trans_conds)
			FSM_INIT_COND (CLOSE)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 10, state10_enter_exec, SEND_FIN;, "CLOSE", "SEND_FIN", "CLOSE_WAIT", "LAST_ACK", "tcp_conn_v3_dt [CLOSE_WAIT -> LAST_ACK : CLOSE / SEND_FIN]")
				FSM_CASE_TRANSIT (1, 9, state9_enter_exec, ;, "default", "", "CLOSE_WAIT", "CLOSE_WAIT", "tcp_conn_v3_dt [CLOSE_WAIT -> CLOSE_WAIT : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (10, "LAST_ACK", state10_enter_exec, "tcp_conn_v3_dt [LAST_ACK enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LAST_ACK enter execs]", state10_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (21,"tcp_conn_v3_dt")


			/** state (LAST_ACK) exit executives **/
			FSM_STATE_EXIT_UNFORCED (10, "LAST_ACK", "tcp_conn_v3_dt [LAST_ACK exit execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LAST_ACK exit execs]", state10_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [LAST_ACK trans conditions]", state10_trans_conds)
			FSM_INIT_COND (RCV_ACK_OF_FIN)
			FSM_DFLT_COND
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 11, state11_enter_exec, ;, "RCV_ACK_OF_FIN", "", "LAST_ACK", "CLOSED", "tcp_conn_v3_dt [LAST_ACK -> CLOSED : RCV_ACK_OF_FIN / ]")
				FSM_CASE_TRANSIT (1, 10, state10_enter_exec, ;, "default", "", "LAST_ACK", "LAST_ACK", "tcp_conn_v3_dt [LAST_ACK -> LAST_ACK : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_UNFORCED (11, "CLOSED", state11_enter_exec, "tcp_conn_v3_dt [CLOSED enter execs]")
				FSM_PROFILE_SECTION_IN ("tcp_conn_v3_dt [CLOSED enter execs]", state11_enter_exec)
				{
			...
			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (23,"tcp_conn_v3_dt")


			/** state (CLOSED) exit executives **/
			FSM_STATE_EXIT_UNFORCED (11, "CLOSED", "tcp_conn_v3_dt [CLOSED exit execs]")
...
void
_op_tcp_conn_v3_dt_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (_op_tcp_conn_v3_dt_diag ())
...
void
_op_tcp_conn_v3_dt_terminate (OP_SIM_CONTEXT_ARG_OPT)
	{
	...
	FIN_MT (_op_tcp_conn_v3_dt_terminate ())
...
/* local variable prs_ptr in _op_tcp_conn_v3_dt_svar function. */
#undef tcb_ptr
...
VosT_Obtype
_op_tcp_conn_v3_dt_init (int * init_block_ptr)
	{
	...
	FIN_MT (_op_tcp_conn_v3_dt_init (init_block_ptr))

	obtype = Vos_Define_Object_Prstate ("proc state vars (tcp_conn_v3_dt)",
		sizeof (tcp_conn_v3_dt_state));
	*init_block_ptr = 0;
...
VosT_Address
_op_tcp_conn_v3_dt_alloc (VOS_THREAD_INDEX_ARG_COMMA VosT_Obtype obtype, int init_block)
	{

#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	tcp_conn_v3_dt_state * ptr;
	FIN_MT (_op_tcp_conn_v3_dt_alloc (obtype))

	ptr = (tcp_conn_v3_dt_state *)Vos_Alloc_Object_MT (VOS_THREAD_INDEX_COMMA obtype);
	if (ptr != OPC_NIL)
		{
		ptr->_op_current_block = init_block;
#if defined (OPD_ALLOW_ODB)
		ptr->_op_current_state = "tcp_conn_v3_dt [init enter execs]";
#endif
		}
...
void
_op_tcp_conn_v3_dt_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	tcp_conn_v3_dt_state		*prs_ptr;

	FIN_MT (_op_tcp_conn_v3_dt_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (tcp_conn_v3_dt_state *)gen_ptr;
